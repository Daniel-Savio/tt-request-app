...
export function Output({ index, remove, data, selectedIedsFromInput }: { index: number, remove: (index: number) => void, data: any, selectedIedsFromInput: { name: string; address: string; modules?: string; comment?: string }[] }) {
  const { control, register, watch, setValue } = useFormContext();
  const protocolo = watch(`saidas.${index}.protocolo`);
  const port = watch(`saidas.${index}.port`);
  const ip = watch(`saidas.${index}.ip`)
  ...
  const filteredSaidas = data?.entradas.filter((saida: string) => {
    const isAlreadySelected = allSaidas.some((e: any, i: number) => i !== index && e.type === saida);

    if (isAlreadySelected && (saida === "71-72" || saida === "74-75")) {
      return false;
    }

    const is7172Selected = allSaidas.some((e: any, i: number) => i !== index && e.type === "71-72");
    const is7475Selected = allSaidas.some((e: any, i: number) => i !== index && e.type === "74-75");
    const is717273Selected = allSaidas.some((e: any, i: number) => i !== index && e.type === "71-72-73");
  ...
  return (
    <div className="flex flex-col gap-2 ml-6 mt-4">
      ...
      <Select
        onValueChange={(value) =>
          setValue(`saidas.${index}.protocolo`, value)
        }
        value={protocolo}
      >
      ...
      <Select
        defaultValue="TCP/IP"
        onValueChange={(value) =>
          setValue(`saidas.${index}.type`, value)
        }
      >
        <SelectTrigger className="w-full">
      ...
      {saidaType && saidaType !== "TCP/IP" && (
        <div className="flex gap-2">
          <Select
            ...
            value={baudRate}
            onValueChange={(value) =>
              setValue(`saidas.${index}.baudRate`, value)
            }
          >
            <SelectTrigger className="w-full">
          ...
          <Select
            ...
            value={dataBits}
            onValueChange={(value) =>
              setValue(`saidas.${index}.dataBits`, value)
            }
          >
            <SelectTrigger className="w-full">
          ...
          <Select
            ...
            value={parity}
            onValueChange={(value) =>
              setValue(`saidas.${index}.parity`, value)
            }
          >
            <SelectTrigger className="w-full">
          ...
          <Select
            ...
            value={stopBits}
            onValueChange={(value) =>
              setValue(`saidas.${index}.stopBits`, value)
            }
          >
            <SelectTrigger className="w-full">
      ...
      <Separator />
      <InputIedSidebar
        ieds={selectedIedsFromInput}
        onAddIed={appendIed}
      />
      <IedList fieldName="saidas" entradaIndex={index} ieds={ieds} onRemoveIed={removeIed} />
